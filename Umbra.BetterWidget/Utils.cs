...
{
    public static bool Enabled      => GetToolbarConfigFieldValue<bool>("Enabled");
    public static bool IsTopAligned => GetToolbarConfigFieldValue<bool>("IsTopAligned");
    ...
    /// <summary>
    /// Retrieves the value of a public static field from the Toolbar class by its name, as a generic type.
    /// </summary>
    ...
    /// <exception cref="TypeLoadException">Thrown when the Toolbar type cannot be loaded.</exception>
    ...
    private static T GetToolbarConfigFieldValue<T>(string fieldName)
    {
        ...
        Assembly asm = Framework.Assemblies
                .FirstOrDefault(a => a.GetType("Umbra.Toolbar", false) != null)
            ?? throw new InvalidOperationException("Assembly containing type Umbra.Toolbar not found.");

        Type toolbarConfigType = asm.GetType("Umbra.Toolbar", throwOnError: false)
        ...
        var field = toolbarConfigType.GetProperty(fieldName, BindingFlags.Public | BindingFlags.Static);
